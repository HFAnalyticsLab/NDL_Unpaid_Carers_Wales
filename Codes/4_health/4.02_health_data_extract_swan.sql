...
DROP TABLE SAILW1429V.LB_CMS_CANCER_LKUP_SWANSEA;
DROP TABLE SAILW1429V.LB_CMS_CKD_LKUP_SWANSEA_2;
DROP TABLE SAILW1429V.LB_CMS_MEDCODE_LKUP_SWANSEA;
DROP TABLE SAILW1429V.LB_CMS_PRODCODE_LKUP_SWANSEA;


/* -- ONLY RUN IF CAM_CODELIST TABLE JUST IMPORTED (should have done in R but didn't realise)
...
*/
-- 1. CANCER
CREATE TABLE SAILW1429V.LB_CMS_CANCER_LKUP_SWANSEA(
	ALF_PE BIGINT,
...
INSERT INTO SAILW1429V.LB_CMS_CANCER_LKUP_SWANSEA
SELECT 
...
FROM
	...
	FROM
		...
		SELECT
			...
			CANEVENT.EVENT_CD
		FROM
			SAILW1429V.PEJE_MATCHED_COHORT_SWANSEA_2 SWANSEA
			-- 1.3. Join to deduplicated carer list to get all cancer Read code events for carers
		INNER JOIN (
			SELECT
				GP.ALF_PE,
			...
			SWANSEA.ALF_PE = CANEVENT.ALF_PE)
...
-- 2. KIDNEY
CREATE TABLE SAILW1429V.LB_CMS_CKD_LKUP_SWANSEA_2(
	ALF_PE BIGINT,
	FIRST_IDENTIFIED_DATE DATE,
	COND VARCHAR(50), 
    REF VARCHAR(50),
...
INSERT INTO SAILW1429V.LB_CMS_CKD_LKUP_SWANSEA_2
SELECT 
	ALF_PE,
	FIRST_IDENTIFIED_DATE,
	COND,
...
FROM
	...
	FROM
		...
		SELECT
			...
			ROW_NUMBER() OVER(PARTITION BY SWANSEA.ALF_PE ORDER BY SWANSEA.ALF_PE, EVENT_DT DESC, "SEQUENCE" DESC) AS RN
		FROM
			SAILW1429V.PEJE_MATCHED_COHORT_SWANSEA_2 SWANSEA
			-- 2.3. Join to deduplicated carer list to get all CKD Read code events for carers
		INNER JOIN (
			...
			SWANSEA.ALF_PE = CKDEVENT.ALF_PE
		-- 2.4 Select records that occured before being identified as an unpaid carer
		WHERE CKDEVENT.EVENT_DT < SWANSEA.FIRST_IDENTIFIED_DATE)
...
-- 3. ALL OTHER 'MEDCODES' 
CREATE TABLE SAILW1429V.LB_CMS_MEDCODE_LKUP_SWANSEA(
	ALF_PE BIGINT,
	FIRST_IDENTIFIED_DATE DATE,
	COND VARCHAR(50), 
    REF VARCHAR(50),
...
INSERT INTO SAILW1429V.LB_CMS_MEDCODE_LKUP_SWANSEA
SELECT 
...
FROM 
	...
	INNER JOIN (
		...
		SWANSEA.ALF_PE = MEDEVENT.ALF_PE
	-- 3.5 ...where event_dt between first_identified_date - read and first_identified_date - 1
	WHERE MEDEVENT.EVENT_DT BETWEEN SWANSEA.FIRST_IDENTIFIED_DATE - MEDEVENT."READ" YEARS AND SWANSEA.FIRST_IDENTIFIED_DATE - 1 DAYS);


-- 4. PRODCODES
CREATE TABLE SAILW1429V.LB_CMS_PRODCODE_LKUP_SWANSEA(
	ALF_PE BIGINT,
	FIRST_IDENTIFIED_DATE DATE,
	COND VARCHAR(50), 
    REF VARCHAR(50),
...
INSERT INTO SAILW1429V.LB_CMS_PRODCODE_LKUP_SWANSEA
SELECT 
	ALF_PE,
	FIRST_IDENTIFIED_DATE,
	COND,
	...
	FROM
		...
		INNER JOIN (
			...
			INNER JOIN (
				...
				ON PRODEVENT.ALF_PE = SWANSEA.ALF_PE 
		WHERE
			-- 4.5 ...where condition is scz and readcode ever recorded...
			(COND = 'SCZ' AND PRODEVENT.EVENT_DT < SWANSEA.FIRST_IDENTIFIED_DATE) OR
			-- 4.6 ...or condition not scz and event_dt between first_identified_date - 365 and first_identified_date - 1 
			(COND != 'SCZ' AND PRODEVENT.EVENT_DT BETWEEN SWANSEA.FIRST_IDENTIFIED_DATE - 1 YEARS AND SWANSEA.FIRST_IDENTIFIED_DATE - 1 DAYS))
	GROUP BY 
		ALF_PE,
		FIRST_IDENTIFIED_DATE,
		COND,
