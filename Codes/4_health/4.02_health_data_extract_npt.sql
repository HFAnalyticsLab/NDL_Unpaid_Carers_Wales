...
DROP TABLE SAILW1429V.LB_CMS_CANCER_LKUP_NPT;
DROP TABLE SAILW1429V.LB_CMS_CKD_LKUP_NPT;
DROP TABLE SAILW1429V.LB_CMS_MEDCODE_LKUP_NPT;
DROP TABLE SAILW1429V.LB_CMS_PRODCODE_LKUP_NPT;
...
-- 1. CANCER
CREATE TABLE SAILW1429V.LB_CMS_CANCER_LKUP_NPT(
	ALF_PE BIGINT,
...
INSERT INTO SAILW1429V.LB_CMS_CANCER_LKUP_NPT
SELECT 
...
FROM
	...
	FROM
		...
		SELECT
			...
			CANEVENT.EVENT_CD
		FROM
			SAILW1429V.PEJE_MATCHED_COHORT_NPT_2 NPT
			-- 1.3. Join to deduplicated carer list to get all cancer Read code events for carers
		INNER JOIN (
			SELECT
				GP.ALF_PE,
...
-- 2. KIDNEY
CREATE TABLE SAILW1429V.LB_CMS_CKD_LKUP_NPT(
	ALF_PE BIGINT,
	FIRST_IDENTIFIED_DATE DATE,
	COND VARCHAR(50), 
	COND_DESC VARCHAR (500),
...
INSERT INTO SAILW1429V.LB_CMS_CKD_LKUP_NPT
SELECT 
	ALF_PE,
	FIRST_IDENTIFIED_DATE,
	COND,
...
FROM
	...
	FROM
		...
		SELECT
			...
			ROW_NUMBER() OVER(PARTITION BY NPT.ALF_PE ORDER BY NPT.ALF_PE, EVENT_DT DESC, "SEQUENCE" DESC) AS RN
		FROM
			SAILW1429V.PEJE_MATCHED_COHORT_NPT_2 NPT
			-- 2.3. Join to deduplicated carer list to get all CKD Read code events for carers
		INNER JOIN (
			...
			NPT.ALF_PE = CKDEVENT.ALF_PE
		-- 2.4 Select records that occured before being identified as an unpaid carer
		WHERE CKDEVENT.EVENT_DT < NPT.FIRST_IDENTIFIED_DATE)
...
-- 3. ALL OTHER 'MEDCODES' 
CREATE TABLE SAILW1429V.LB_CMS_MEDCODE_LKUP_NPT(
	ALF_PE BIGINT,
	FIRST_IDENTIFIED_DATE DATE,
	COND VARCHAR(50),
	COND_DESC VARCHAR(500),
...
INSERT INTO SAILW1429V.LB_CMS_MEDCODE_LKUP_NPT
SELECT 
...
FROM 
	...
	INNER JOIN (
		...
		NPT.ALF_PE = MEDEVENT.ALF_PE
	-- 3.5 ...where event_dt between first_identified_date - read and first_identified_date - 1
	WHERE MEDEVENT.EVENT_DT BETWEEN NPT.FIRST_IDENTIFIED_DATE - MEDEVENT."READ" YEARS AND NPT.FIRST_IDENTIFIED_DATE - 1 DAYS);


-- 4. PRODCODES
CREATE TABLE SAILW1429V.LB_CMS_PRODCODE_LKUP_NPT(
	ALF_PE BIGINT,
	FIRST_IDENTIFIED_DATE DATE,
	COND VARCHAR(50), 
	COND_DESC VARCHAR(500),
...
INSERT INTO SAILW1429V.LB_CMS_PRODCODE_LKUP_NPT
SELECT 
	ALF_PE,
	FIRST_IDENTIFIED_DATE,
	COND,
	...
	FROM
		...
		INNER JOIN (
			...
			INNER JOIN (
				...
				ON PRODEVENT.ALF_PE = NPT.ALF_PE 
		WHERE
			-- 4.5 ...where condition is scz and readcode ever recorded...
			(COND = 'SCZ' AND PRODEVENT.EVENT_DT < NPT.FIRST_IDENTIFIED_DATE) OR
			-- 4.6 ...or condition not scz and event_dt between first_identified_date - 365 and first_identified_date - 1 
			(COND != 'SCZ' AND PRODEVENT.EVENT_DT BETWEEN NPT.FIRST_IDENTIFIED_DATE - 1 YEARS AND NPT.FIRST_IDENTIFIED_DATE - 1 DAYS))
	GROUP BY 
		ALF_PE,
		FIRST_IDENTIFIED_DATE,
		COND,
