-------------------------------------------------------------
/* Swansea GP Interactions: 1:1 Match */
-------------------------------------------------------------
...
DROP TABLE SAILW1429V.GBH_MATCHEDCOHORT_GP_SWANSEA_STEP1 ;
DROP TABLE SAILW1429V.GBH_MATCHEDCOHORT_GP_SWANSEA ;

-- Create step 1 table
CREATE TABLE SAILW1429V.GBH_MATCHEDCOHORT_GP_SWANSEA_STEP1 (
ALF_PE BIGINT, 
SEX INTEGER, 
AGE INTEGER,
WOB DATE,
...
INSERT INTO SAILW1429V.GBH_MATCHEDCOHORT_GP_SWANSEA_STEP1 
-- 1. Get WLGP info for those with an interaction in the year prior to being identified as an unpaid carer 
...
WHERE 
	...
	GP.EVENT_DT BETWEEN (S.FIRST_IDENTIFIED_DATE - 1 YEAR) AND (S.FIRST_IDENTIFIED_DATE - 1 DAY)	
...
FROM SAILW1429V.GBH_MATCHEDCOHORT_GP_SWANSEA_STEP1
GROUP BY ALF_PE, EVENT_DT
HAVING COUNT(*) >1; 

CREATE TABLE SAILW1429V.GBH_MATCHEDCOHORT_GP_SWANSEA (
	ALF_PE BIGINT,
	SEX INTEGER,
	SEX_DESC VARCHAR(10),
	AGE INTEGER,
...
INSERT INTO SAILW1429V.GBH_MATCHEDCOHORT_GP_SWANSEA
SELECT DISTINCT *
FROM(
	SELECT
		DISTINCT S.ALF_PE,
	...
	LEFT JOIN (SELECT * 
		FROM SAILW1429V.GBH_MATCHEDCOHORT_GP_SWANSEA_STEP1) B
...
SELECT 
	COUNT(*)
FROM 
	SAILW1429V.GBH_MATCHEDCOHORT_GP_SWANSEA; 
-- Carer count
SELECT 
	COUNT(UNIQUE(ALF_PE))
FROM 
...
FROM SAILW1429V.GBH_MATCHEDCOHORT_GP_SWANSEA
GROUP BY CARER_FLAG; 


-- check how many individuals do not have a GP interaction 
SELECT COUNT(*) 
FROM SAILW1429V.GBH_MATCHEDCOHORT_GP_SWANSEA
