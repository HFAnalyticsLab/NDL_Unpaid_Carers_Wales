-------------------------------------------------------------
/* DENBIGHSHIRE Inpatients */
-------------------------------------------------------------

/* STEPS:
 * 1. Get PEDW info for those with a planned/emergency admission in the year prior to being identified as a carer
 * 	- 1.1. Select main variables from deduplicated table (PEJE_MATCHED_COHORT_DENBIGHSHIRE_2_2)
...
DROP TABLE SAILW1429V.PEJE_MATCHEDCOHORT_INPATIENT_DENBIGHSHIRE_2_STEP1;

CREATE TABLE SAILW1429V.PEJE_MATCHEDCOHORT_INPATIENT_DENBIGHSHIRE_2_STEP1
(ALF_PE BIGINT
, SEX INTEGER
, AGE INTEGER
, WOB DATE
...
INSERT INTO SAILW1429V.PEJE_MATCHEDCOHORT_INPATIENT_DENBIGHSHIRE_2_STEP1
-- 1. Get PEDW info for those with a planned/emergency admission in the year prior to being identified as a carer
-- 1.1. Select main variables from deduplicated table (PEJE_MATCHED_COHORT_DENBIGHSHIRE_2_2)
SELECT
	DISTINCT J.ALF_PE,
	...
	PEDW.DISCH_DT
FROM
	SAILW1429V.PEJE_MATCHED_COHORT_DENBIGHSHIRE_2 J
-- 1.2. Left join to PEDW 
...
DROP TABLE SAILW1429V.PEJE_MATCHEDCOHORT_INPATIENT_DENBIGHSHIRE_2;

CREATE TABLE SAILW1429V.PEJE_MATCHEDCOHORT_INPATIENT_DENBIGHSHIRE_2 (
	ALF_PE BIGINT,
	SEX INTEGER,
	SEX_DESC VARCHAR(10),
	AGE INTEGER,
...
INSERT	INTO SAILW1429V.PEJE_MATCHEDCOHORT_INPATIENT_DENBIGHSHIRE_2
SELECT 	DISTINCT *
FROM (
	SELECT
		DISTINCT J.ALF_PE,
	...
	FROM
		SAILW1429V.PEJE_MATCHED_COHORT_DENBIGHSHIRE_2 J
	-- 2.Join deduplicated carer table to PEDW info for those with a planned/emergency admission in the year prior to being identified as a carer from step 1
	LEFT JOIN (SELECT *  FROM SAILW1429V.PEJE_MATCHEDCOHORT_INPATIENT_DENBIGHSHIRE_2_STEP1 ) B 
