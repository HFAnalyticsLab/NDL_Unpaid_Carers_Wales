 ...
 * - Known sex (1 or 2)
 * - Resides in NPT, Gwynedd, Swansea or Denbighshire on event date (Also accounts for known deprivation) - LSOA codes in SAILW1429V.LB_LSOA_LA_LOOKUP
 ...
 * 2. LSOA Denbighshire in lookup table 
...
DROP TABLE SAILW1429V.LB_GPCARER_DEN;

--STEP 1
CREATE TABLE SAILW1429V.LB_GPCARER_STEP1_DEN (ALF_PE BIGINT,
START_DATE DATE,
END_DATE DATE,
...
INSERT
	INTO
	SAILW1429V.LB_GPCARER_STEP1_DEN
-- 3. Link to WDSD demographics
 SELECT
	DISTINCT B.*,
	R.RUC11,
...
FROM
	(
	-- 2. LSOA in lookup table as Denbighshire
	SELECT
		A.*,
		L.LA_NAME
	FROM
...
-- STEP 2
 CREATE TABLE SAILW1429V.LB_GPCARER_STEP2_DEN (ALF_PE BIGINT,
START_DATE DATE,
END_DATE DATE,
LSOA2011_CD VARCHAR(10),
WIMD_2019_QUINTILE INTEGER,
...
INSERT
	INTO
	SAILW1429V.LB_GPCARER_STEP2_DEN
SELECT
	DISTINCT A.*,
	GP.EVENT_DT
FROM
	SAILW1429V.LB_GPCARER_STEP1_DEN A
	--5. Link to WLGP (inner join)
INNER JOIN SAIL1429V.WLGP_GP_EVENT_CLEANSED_20220301 GP ON
	...
	AND GP.EVENT_DT BETWEEN '2020-04-01' AND '2022-03-03';


-- STEP 3
 CREATE TABLE SAILW1429V.LB_GPCARER_STEP3_DEN (ALF_PE BIGINT,
START_DATE DATE,
END_DATE DATE,
LSOA2011_CD VARCHAR(10),
WIMD_2019_QUINTILE INTEGER,
...
-- 6. Link to ADDE
 INSERT
	INTO
	SAILW1429V.LB_GPCARER_STEP3_DEN
SELECT
	A.*,
	D.DEATH_DT
FROM
	SAILW1429V.LB_GPCARER_STEP2_DEN A
...
-- 8. Calculate age on EVENT_DT
 ALTER TABLE SAILW1429V.LB_GPCARER_STEP3_DEN ADD COLUMN AGE INTEGER;
...
-- STEP 4
 CREATE TABLE SAILW1429V.LB_GPCARER_STEP4_DEN (ALF_PE BIGINT,
...
--10. Link to CLEAN_ADD_WALES
 INSERT
	INTO
	SAILW1429V.LB_GPCARER_STEP4_DEN
SELECT
	DISTINCT A.*,
	W.START_DATE,
	W.END_DATE,
	W.WELSH_ADDRESS
FROM
	SAILW1429V.LB_GPCARER_STEP3_DEN A
...
-- STEP 5
CREATE TABLE SAILW1429V.LB_GPCARER_STEP5_DEN (ALF_PE BIGINT,
LSOA_START_DATE DATE,
LSOA_END_DATE DATE,
LSOA2011_CD VARCHAR(10),
WIMD_2019_QUINTILE INTEGER,
...
-- 12. Link to WLGP SAIL practices
 INSERT
	INTO
	SAILW1429V.LB_GPCARER_STEP5_DEN
SELECT
	A.*
FROM
	SAILW1429V.LB_GPCARER_STEP4_DEN A
...
-- 14. Group by ALF, order by event date, add row number to indicate first event when multiple
 ALTER TABLE SAILW1429V.LB_GPCARER_STEP5_DEN ADD COLUMN ROW_NUM INTEGER;
...
-- 16. Create final table
 CREATE TABLE SAILW1429V.LB_GPCARER_DEN LIKE SAILW1429V.LB_GPCARER_STEP5_DEN;

INSERT
	INTO
	SAILW1429V.LB_GPCARER_DEN
SELECT
	*
FROM
	SAILW1429V.LB_GPCARER_STEP5_DEN;
...
-- 17. DROP interim tables
DROP TABLE SAILW1429V.LB_GPCARER_STEP1_DEN;

DROP TABLE SAILW1429V.LB_GPCARER_STEP2_DEN;

DROP TABLE SAILW1429V.LB_GPCARER_STEP3_DEN;

DROP TABLE SAILW1429V.LB_GPCARER_STEP4_DEN;

DROP TABLE SAILW1429V.LB_GPCARER_STEP5_DEN;
